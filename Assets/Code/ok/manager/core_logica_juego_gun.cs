...
public class core_logica_juego_gun : MonoBehaviour
{
    ...
    public static int dinero_jugador, score_jugador;
    ...
    public GameObject paneldesenfoque, pausaBtn;
...
////////////////////////
    ...
    void Start()
    {
        ...
        Time.timeScale = 1;
        UiManager.ui_manager_instance.dialogo_hero_tmp.text = "";
        UiManager.ui_manager_instance.cuadro_dialogo_hero.SetActive(false);
    ...
    public void addmedicos()
    {
        if (game_over == false)
        {
            if (dinero_jugador >= 15)
            {
                ...
                invocomedico = true;
                UiManager.ui_manager_instance.speak_hero("UN MÉDICO!!!");
    ...
    public void addwarlord()
    {
        if (game_over == false)
        {
            if (dinero_jugador >= 25)
            {
                ...
                invocowarlord = true;
                UiManager.ui_manager_instance.speak_hero("NECESITO MUNICIONES");
    ...
    public void addmilitar()
    {
        ...
        if (game_over == false)
        {
            if (dinero_jugador >= 10)
            {
                ...
                invocomilitar = true;
                UiManager.ui_manager_instance.speak_hero("REFUERZOS!!!");
    ...
    public void GameOver(string textoderrota)
    {
        if (win_state == false)
        {
            ...
            Overtextexplain.SetText(textoderrota);
             GameManager.gm_instance.cargarDatos();
            if (GameManager.gm_instance.score_juego_maximo < score_jugador) { GameManager.gm_instance.score_juego_maximo = score_jugador;  GameManager.gm_instance.salvarDatos(); }
        }
    }
    ...
    public void WinOver()
    {
        ...
        win_state = true;
        GameManager.gm_instance.cargarDatos();
        if (GameManager.gm_instance.score_juego_maximo < score_jugador) { GameManager.gm_instance.score_juego_maximo = score_jugador;}
...
/////////funciones sencillas///////
   public void sumarScore(int puntos_obtenidos)
    {
        score_jugador = score_jugador + puntos_obtenidos;
        UiManager.ui_manager_instance.score_tmp.text = "Score: " + score_jugador.ToString();
    }

    public void sumarDinero(int dd)
    {
       AudioManager.audio_manager_instance.play_one_shot("sonido_dinero",0.5f); 
       dinero_jugador = dinero_jugador + dd;
       UiManager.ui_manager_instance.dinero_tmp.text = dinero_jugador.ToString() + "$";
    }
