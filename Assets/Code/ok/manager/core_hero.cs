...
public class core_hero : MonoBehaviour
{
   ...
   //Audio
   AudioSource hero_audio_source;
   [SerializeField]
   AudioClip shot_sound,no_bullets_sound,helicopter_sound, car_sound,money_sound;
    ...
    void Start()
    {
        ...
        hero_audio_source = GetComponent<AudioSource>();
...
void seleccion_piso_finalizada()
{
        ...
        if (core_logica_juego_gun.core_logica_juego_gun_instance.helicoptero_state == true) {
            ...
            play_one_shot("sonido_helicoptero",1);
            }
            seleccion_piso_grupo.SetActive(false);
...
////////////////////FIN DEL VIAJE DEL HEROE//////////////////////////
    ...
    void disparoHero()
    {
        ...
        if (cantidad_balas > 0 && estado_invisible_heroe==false)
        {
            ...
            disparofuego.GetComponent<SpriteRenderer>().color = new Color32(255, 255, 255, 255);
            play_one_shot("disparo_hero",0.2f);
...
void animacion_pasar_piso()
      ...
      public  void derecha_btn_dwn()
    {
       ...
       if(menu_movimiento_piso==true) {pasar_piso_derecha(); AudioManager.audio_manager_instance.play_one_shot("para_texto",1);}
       
       else if(menu_movimiento_tropas){core_logica_juego_gun.core_logica_juego_gun_instance.tropa_siguiente_btn(); AudioManager.audio_manager_instance.play_one_shot("para_texto",1);}
    ...
    public void izquierda_btn_dwn() {
        ...
        if(menu_movimiento_piso==true) {pasar_piso_izquierda(); AudioManager.audio_manager_instance.play_one_shot("para_texto",1); }
            else if(menu_movimiento_tropas){ core_logica_juego_gun.core_logica_juego_gun_instance.tropa_anterior_btn(); AudioManager.audio_manager_instance.play_one_shot("para_texto",1);}
        ...
        switch(position_boton_lugar)
       {
        ...
        //////////////FIN ONLY MANDO///////////////
             ...
             if (core_logica_juego_gun.core_logica_juego_gun_instance.vendedor_armas_state == true)   
            {
                ...
                if (cantidaddinero < 3)
                {
                     UiManager.ui_manager_instance.speak_hero("NO HAY RECURSOS!!!");
                }
                else
                {
                    if (cantidad_balas == 45)
                    {
                         UiManager.ui_manager_instance.speak_hero("TENGO BALAS");
                    }
                    else
                    {
                        sumarBalas(20);
            ...
            {
                if (cantidad_balas == 45) {     UiManager.ui_manager_instance.speak_hero("TENGO BALAS");}
             ...
             if (core_logica_juego_gun.core_logica_juego_gun_instance.cocinero_state == true)
            {
                ...
                else
                {
                    ...
                    {
                          UiManager.ui_manager_instance.speak_hero("ESTOY BIEN");
                    }
                    else
                    {
...
public int vidaHeroMax() { return mi_hero.getVidaMaxPlayer(); }
  ...
  public void play_one_shot(string nombre_cancion, float volume)
   {
    hero_audio_source.volume=volume; 
     switch(nombre_cancion)
     {
      case "sonido_carro":  hero_audio_source.PlayOneShot(car_sound); break; 
      case "sonido_helicoptero":  hero_audio_source.PlayOneShot(helicopter_sound); break; 
      case "disparo_hero":  hero_audio_source.PlayOneShot(shot_sound); break; 
      case "sonido_dinero":  hero_audio_source.PlayOneShot(money_sound); break; 
      case "sin_balas":  hero_audio_source.PlayOneShot(no_bullets_sound); break; 
     }
   }
